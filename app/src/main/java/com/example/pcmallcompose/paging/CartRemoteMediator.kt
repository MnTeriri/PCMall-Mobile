...
class CartRemoteMediator(
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, Cart>): MediatorResult {
        try {
            val loadKey = when (loadType) {
                ...
                APPEND -> {
                    val remoteKey = database.withTransaction {
                        remoteKeyDao.remoteKeyByQuery(TABLE_NAME, query)
                    }
            ...
            val response = cartService.searchAllCart(query, loadKey, state.config.pageSize)
            ...
            database.withTransaction {
                if (loadType == REFRESH) {
                    remoteKeyDao.deleteByQuery(TABLE_NAME, query)
                    cartDao.clearAll()
                }
                // 更新该查询的 RemoteKey。
                remoteKeyDao.insertOrReplace(RemoteKey(TABLE_NAME, query, loadKey))
